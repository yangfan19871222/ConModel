...
class FaceDataIter(mx.io.DataIter):
    ...
    def provide_label(self):
        ...
        return  [('yaw_label', (self.batchsize, 1)),\
                 ('pitch_label',  (self.batchsize, 1)),\
                 ('roll_label',  (self.batchsize, 1))]
    ...
    def __iter__(self):
        while self.cur_iter < self.max_iter:
            ...
            yield FaceBatch(['data'], [self.data], ['yaw_label', 'pitch_label', 'roll_label'], self.total_label_tmp) 
            #print 'test'
           # if landmarks_task == True:
...
#            self.f.seek(0, 0)
       # for i in range(self.batchsize):
        ...
        while i < self.batchsize:
           ...
           #         continue
           #     if float(cur_file_list[2]) < 35 and float(cur_file_list[2]) > 20 and float(cur_file_list[3]) < 10:
           #         continue
                   # non_face_rand_kid += 1
           ...
           # if image.shape[1] < 60 or image.shape[0] < 60:
           #     if float(cur_file_list[2]) < 30: 
           #         less_than80_face += 1

           #         if less_than80_face % 5 != 0:
           #             continue
            #print file_path
            height = image.shape[0]
            width = image.shape[1]

            self.yaw_label.append([float(cur_file_list[-3]) / 90.])
            self.pitch_label.append([float(cur_file_list[-2]) / 90.])
            self.roll_label.append([float(cur_file_list[-1]) / 90.])
            #print cur_file_list[-3]
#            image = image[int(0.1 * height): int(0.9 * height), int(0.1 * width): int(0.9 * width)]
           # resize_face += 1
            #if resize_face % 2 == 0:
                #image = cv2.resize(image, (48, 48))
            #image = cv2.GaussianBlur(image, (5, 5), 1.5)
          #  if image.shape[1] > 80 and image.shape[0] > 80:
                
          #      image = cv2.resize(image, (height / 8, width / 8))
...
#                self.landmarks.append([float(cur_file_list[6]), float(cur_file_list[7]), float(cur_file_list[8]), float(cur_file_list[9]),\
 ...
 #                   float(cur_file_list[14]), float(cur_file_list[15].strip('\n'))])
        ...
        self.data = mx.nd.array(self.data)
        self.yaw_label = mx.nd.array(self.yaw_label)
        self.pitch_label = mx.nd.array(self.pitch_label)
        self.roll_label = mx.nd.array(self.roll_label)
...
#            self.total_label = [self.label, self.outside_weights, self.bbox_targets]
        self.total_label.append([self.yaw_label, self.pitch_label, self.roll_label])
        self.total_data.append(self.data)
        self.total_label_tmp = [self.yaw_label, self.pitch_label, self.roll_label]
         

def get_symbol():
    data = mx.symbol.Variable('data')
    #gender_label = mx.symbol.Variable('gender_label')
    yaw_label = mx.symbol.Variable('yaw_label')
    pitch_label = mx.symbol.Variable('pitch_label')
    roll_label = mx.symbol.Variable('roll_label')
   ...
   # if attractive_task == True:
    ...
    loss1_ = mx.symbol.square(data = (conv6_1 - yaw_label))
    loss1_ = mx.symbol.sum(data = loss1_, axis = (1))
    loss1 = mx.symbol.MakeLoss(data = loss1_, name = 'loss1', grad_scale = 200.0, normalization = 'valid')

    conv6_2 = mx.symbol.FullyConnected(data=relu5, num_hidden = 1, name = 'conv6_2')
    loss2_ = mx.symbol.square(data = (conv6_2 - pitch_label))
    loss2_ = mx.symbol.sum(data = loss2_, axis = (1))
    loss2 = mx.symbol.MakeLoss(data = loss2_, name = 'loss2', grad_scale = 200.0, normalization = 'valid')
            
    conv6_3 = mx.symbol.FullyConnected(data=relu5, num_hidden = 1, name = 'conv6_3')
    loss3_ = mx.symbol.square(data = (conv6_3 - roll_label))
    loss3_ = mx.symbol.sum(data = loss3_, axis = (1))
    loss3 = mx.symbol.MakeLoss(data = loss3_, name = 'loss3', grad_scale = 200.0, normalization = 'valid')
...
#             mx.metric.check_label_shapes(label, pred_label)
              ...
              pred = preds[i].asnumpy().astype('float32')
              label = labels[i].asnumpy().astype('float32')
              self.sum_metric[i] += np.sum(pred)
                #for t in range(len(label)):
               #     if label[t, 0] != 0:
               #         self.num_inst[i] += 1
              self.num_inst[i] += pred.shape[0]
...
if __name__ == '__main__':
    ...
    val = FaceDataIter(batch_size, os.path.join(root_path, './imglist_stage4/train'), (3, 48, 48), 12800, begin = 1000000)
    device = mx.gpu(7)
    lr = 0.01

    #params = 'mtcnn_smaller-0010.params'
   
    params = 'mtcnn_online_three_grad100-0100.params'
    #params = 'mtcnn_online_output2_35_glass_resize_bigger_96x96_bigger-0002.params'
  ...
  #      label_num = 1
    ...
    model.save('mtcnn_online_three_grad200')
